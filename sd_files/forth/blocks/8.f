\ === Block 8: Interactive Forth Debugger & Disassembler ===
marker --
use display
variable dbg-pc
variable dbg-h variable dbg-xt
variable dbg-cur variable dbg-top
variable dbg-end variable nav-d
variable dbg-run? variable bp
variable lbuf 64 allot
variable nav-stk 16 allot

\ --- Navigation Stack (in RAM) ---
: nav-push
...
\ --- Dictionary Header Helpers ---
: h>len 1+ dict@ 63 and ;
: h>xt dup 2 + swap h>len 1 + 2 / + ;
: .wname dup 1+ dict@ 63 and
0 do dup 2 + i +dict-c@ emit loop drop ;

: pc>h
>r 4 dict@ begin
dup 0= if r> drop exit then
r@ over h>xt >= if r> drop exit then
dict@
again ;
...
>r 4 dict@ begin
dup 0= if r> drop exit then
dup h>xt r@ = if r> drop exit then
dict@
again ;
...
>r 4 dict@ begin
dup 0= if r> drop exit then
dup 1+ dict@ 64 and if
dup h>xt dict@ r@ = if r> drop exit then
then
dict@
again ;
...
>r 4 dict@ begin
dup 0= if r> drop exit then
dup dict@ r@ = if r> drop exit then
dict@
again ;

: word-end
dup next-h dup if nip else drop here then ;

: dbg-set-word
dup dbg-h !
dup h>xt dup dbg-xt ! dbg-top !
word-end dbg-end ! ;
...
: wname+
dup 1+ dict@ 63 and dup if
0 do dup 2 + i +dict-c@ c+ loop drop
else drop drop then ;

: n+
dup 0= if 48 c+ drop exit then
dup <0 if 45 c+ negate then
...
: dlit@ dup 1 + dict@ dup if 16 lshift swap 2 + dict@ or else drop 2 + dict@ then ;

\ --- Instruction Disassembly ---
: fmt-inst
0 bp !
if 62 c+ 32 c+ else 32 c+ 32 c+ then
dup n+ s-col s+
dup dict@
...
0 bp ! s-hdr s+
dbg-h @ wname+
s-lvl s+ nav-d @ n+ s-rb s+ z+
lbuf 0 disp-write-ml ;

: draw-body
dbg-top @
6 0 do
dup dbg-end @ < if
dup dup dbg-cur @ = fmt-inst
lbuf i 1+ 16 * disp-write-ml
next-inst
else
0 bp ! z+
lbuf i 1+ 16 * disp-write-ml
...
: vdraw
adjust-scroll
draw-hdr draw-body draw-bar ;

\ --- Navigation Actions ---
: act-up dbg-xt @ dbg-cur @ prev-inst dbg-cur ! ;

: act-down
dbg-cur @ next-inst dup dbg-end @ < if
dbg-cur !
else
drop
then ;
...
1 dbg-run? !
dbg-pc @ pc>h
dup 0= if 0 dbg-run? ! drop exit then
dbg-set-word
dbg-pc @ dbg-cur ! ;
