...
: ins ( c -- ) cy @ L >r r@ 1+ cx @ br.display.setChar
  cx @ 1+ r@ @ max r@ br.display.setLen 1 0 move rdrop ;
: del ( -- ) -1 0 move 32 cy @ L 1+ cx @ br.display.setChar ;
: edk ( c -- )
...
: edi ( b -- ) dup blk ! lines-buf swap br.block.load
  0 cx ! 0 cy ! 0 sy ! 0 sx !
  begin draw key dup 27 <> while edk scroll repeat
  drop lines-buf blk @ br.block.save cls ; ['] edi is ed
